...
TODO: Automate complete Site catalog configuration
#>
...
for ($i = 0; $i -lt $envs.Count; $i++) {
            if($envs[$i] -eq $environment){
                ...
                Convert-PnPFolderToProvisioningTemplate -Out "SiteCatalogue.pnp" -Folder ""
...
Apply-PnPProvisioningTemplate -Path SiteCatalogue.pnp -Handlers Fields,ContentTypes,Navigation,Pages
...
Apply-PnPProvisioningTemplate -Path SiteCatalogue.pnp -ExcludeHandlers "Fields, ContentTypes, Navigation, Pages"
