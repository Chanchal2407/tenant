try {
	...
	$Config = (Get-Content "VideoPortalSitesConfig.json") | Out-String | ConvertFrom-Json
    ...
    Write-Host "Adding site scripts... " -NoNewline
        ...
        $scriptJson = Get-Content $("VideoPortalSiteDesign\" + $Config.OrganizationSettings.environment + "\VideoPortal-script-joinToHub.json") -Raw
        $script = Get-PnPSiteScript | Where-Object { $_.Title -eq "$ALEnvPrefix Join to VideoPortalSite Hub" }
        if ($script -ne $null) {
            $joinHubscript = Set-PnPSiteScript -Identity $script.Id -Content $scriptJson
            $joinHubscript = $script
        } else {
            $joinHubscript = Add-PnPSiteScript -Title "$ALEnvPrefix Join to VideoPortalSite Hub" -Description "Join site to VideoPortalSite Hub" -Content $scriptJson
        }

		# Add Page Footer
        $scriptJson = Get-Content $("VideoPortalSiteDesign\" + $Config.OrganizationSettings.environment + "\VideoPortal-script-pageFooterExtension.json") -Raw
        $script = Get-PnPSiteScript | Where-Object { $_.Title -eq "$ALEnvPrefix  VideoPortal Page Footer Extension" }
        if ($script -ne $null) {
            $addFooterScript = Set-PnPSiteScript -Identity $script.Id -Content $scriptJson
            $addFooterScript = $script
        } else {
            $addFooterScript = Add-PnPSiteScript -Title "$ALEnvPrefix  VideoPortal Page Footer Extension" -Description " VideoPortal Page Footer Extension" -Content $scriptJson
        }
        ...
        # Apply Clarity for Prod
        if ($Config.OrganizationSettings.environment -eq "PROD")
        {
            # Add Clarity
            $scriptJson = Get-Content $("VideoPortalSiteDesign\" + $Config.OrganizationSettings.environment + "\VideoPortal-script-clarityExtension.json") -Raw
            $script = Get-PnPSiteScript | Where-Object { $_.Title -eq "$ALEnvPrefix Video Portal Microsoft Clarity Extension" }
            if ($script -ne $null) {
                $addClarityScript = Set-PnPSiteScript -Identity $script.Id -Content $scriptJson
                $addClarityScript = $script
            } else {
                $addClarityScript = Add-PnPSiteScript -Title "$ALEnvPrefix Video Portal Microsoft Clarity Extension" -Description "Add Video Portal Microsoft Clarity" -Content $scriptJson
            }
        }
    ...
    # ADD SITE DESIGN
    $newDesignTitle = "$ALEnvPrefix design -  Video Portal site"
    Write-Host "Adding '$newDesignTitle'... " -NoNewline
        ...
        if ($existingDesign -ne $null) {
             # To apply Clarity extension verify if PROD 
            if ($Config.OrganizationSettings.environment -eq "PROD")
            {
            ...
            if ($Config.OrganizationSettings.environment -eq "PROD")
            {
                $sitedesign = Add-PnPSiteDesign -Title $newDesignTitle -WebTemplate "68" -SiteScriptIds $themeScript.ID,$languageScript.ID,$brandingScript.ID,$addGlobalTopNavScript.ID,$joinHubscript.ID,$addFooterScript.ID,$sitePagesScript.ID,$addCriticalInfoScript.ID,$addClarityScript.ID -Description "AlfaLaval  Video Portal site design"
            }else{
                $sitedesign = Add-PnPSiteDesign -Title $newDesignTitle -WebTemplate "68" -SiteScriptIds $themeScript.ID,$languageScript.ID,$brandingScript.ID,$addGlobalTopNavScript.ID,$joinHubscript.ID,$addFooterScript.ID,$sitePagesScript.ID,$addCriticalInfoScript.ID -Description "AlfaLaval  Video Portal site design"
            }
        }
